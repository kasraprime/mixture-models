...
def mnist_loader(data_type,batch_size):
    ...
    if data_type == 'float':
        ...
        train_dataset = MNIST('data/pytorch_data/MNIST', train=True, download=True, transform=transforms.Compose([transforms.ToTensor(), lambda x:x>0.5, lambda x: x.float()]))
        test_dataset = MNIST('data/pytorch_data/MNIST', train=False, download=True, transform=transforms.Compose([transforms.ToTensor(), lambda x:x>0.5, lambda x: x.float()]))
...
def ComputeExpLogPosterior(data_i, component_k, pi, theta, K_mixture, J_parameter_dimension, device):
    ...
    return current_posterior_gamma_ik, log_numerator, log_denominator
    ...
    for epoch in tqdm(range(epoch_num)):
        ...
        for i,data in enumerate(train_loader):
            ...
            for k in range(K_mixture):
                ...
                posterior_gamma_ik, numerator, denominator = ComputeExpLogPosterior(data_i, k, pi, theta, K_mixture, J_parameter_dimension, device)
